...
reportSchema.statics.getReportsWithFilters = function (
  filters = {},
  options = {}
) {
  const { status, category, assignedTo, severity, startDate, endDate, search } =
    filters;
...
reportSchema.methods.addStatusHistory = function (
  status,
  changedBy,
  notes = ""
) {
...
reportSchema.methods.addMessage = function (
  content,
  fromAdmin = false,
  sentBy = null
) {
